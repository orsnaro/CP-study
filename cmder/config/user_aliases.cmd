...
allgitall= set oldPath=C:\Users\OmarPc\repo_CPP_COMP_study $t cd c:\Users\OmarPc\repo_MSP_ML_study $t git add --all $t git commit -m "# $*" $t git push $t cd c:\Users\OmarPc\repo_CPP_COMP_study $t git add --all $t git commit -m "# $*" $t git push $t cd c:\Users\OmarPc\repo_CHROMA_BMP_proj $t git add --all $t git commit -m "# $*" $t git push $t  cd c:\Users\OmarPc\repo_Embedded-C  $t git add --all $t git commit -m "# $*" $t git push $t cd c:\Users\OmarPc\repo_Old_projects_stash $t git add --all $t git commit -m "# $*" $t git push $t cd c:\Users\OmarPc\repo_CRAFTING_INTERPRETERS_study  $t git add --all $t git commit -m "# $*" $t git push $t cd c:\Users\OmarPc\repo_MSP_College_system_mini_proj  $t git add --all $t git commit -m "# $*" $t git push $t cd %oldPath%  
